...
axios.get(INDEX_URL).then(response => {
  originDataArray.origin = response.data.results
...
collection.addEventListener('click', e => {
  ...
  `
    axios.get(showUrl).then(response => {
      renderDetailInfo(response.data)
    }).catch(error => console.log(error))
  }
})
...
function renderPages(data) {
  ...
  currentTotalPage = Math.ceil(data.length / ITEM_PER_PAGE) || InitPage

  for (let i = 0; i < currentTotalPage; i++) {
    paginationContent += `
...
function dragStart(e) {
  ...
  dropTarget.classList.add('text-primary')
  favoriteText.textContent = 'Drag person here!'
}


//監控放置目標
const dropTarget = document.querySelector('#target-container')
...
function dragEnd(e) {
  ...
  dropTarget.classList.remove('text-primary')
  favoriteText.textContent = 'Favorite person here!'
...
function addFavoriteItem(id) {
  const existList = JSON.parse(localStorage.getItem('favoritePerson')) || []
  const person = currentData.find(item => item.id === Number(id))
  let feedbackText = document.getElementById('feedback-text')
  feedbackText.classList.remove('d-none')


  if (existList.some(item => item.id === Number(id))) {
    feedbackText.textContent = `${person.name} is already in your favorite list.`
    feedbackText.classList.add('text-danger')
    feedbackText.classList.remove('text-success')
  } else {
    existList.push(person)
    feedbackText.textContent = `Added ${person.name} success!`
    feedbackText.classList.add('text-success')
    feedbackText.classList.remove('text-danger')
  }
  localStorage.setItem('favoritePerson', JSON.stringify(existList))
}
